          ...
          <div style={{ display: "flex", gap: "12px" }}>
            ...
            <button
              onClick={() => {
                ...
                const topRole = data?.roles?.topRoles?.[0]?.occupation?.title ?? data?.roles?.topRoles?.[0]?.role ?? 'Software Engineer';
                const stage = data?.careerStage?.stage ?? 'student';
                const text = `I just analyzed my resume with ResumeAI Pro and scored ${score}/100.\n\nTop role match: ${topRole}\nCareer stage: ${stage}\n\nGet your free resume diagnostic at resume-ai-pro-psi.vercel.app`;
                const linkedInUrl = `https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent('https://resume-ai-pro-psi.vercel.app')}&summary=${encodeURIComponent(text)}`;
