...
export function buildIntrinsicDeficiencies(input: RecommendationInput): CausalRecommendation[] {
    ...
    if (input.quantifiedMetricsCount < 3) {
        results.push({
            ...
            description: `Your resume has only ${input.quantifiedMetricsCount} quantified result(s). Add metrics like "Reduced latency by 40%", "Served 10k+ users", or "Increased revenue by $50k" to make your impact concrete and compelling to recruiters.`,
            priorityLevel: 'high',
            estimatedEffort: '1-2 hours',
            estimatedImpact: 9,
    ...
    if (input.actionVerbsCount < 5) {
        results.push({
            ...
            description: `Only ${input.actionVerbsCount} action verb(s) detected. Replace passive phrases with strong openers like "Engineered", "Architected", "Launched", "Optimized", or "Delivered" — each bullet should start with a verb that shows ownership and impact.`,
            priorityLevel: 'high',
            estimatedEffort: '30-60 minutes',
            estimatedImpact: 8,
    ...
    if (summaryWordCount > 0 && summaryWordCount < 50) {
        results.push({
            ...
            description: `Your summary is only ~${summaryWordCount} words — too brief to capture a recruiter's attention. Expand it to 3–4 sentences covering your specialization, your top 2–3 skills, and one concrete proof point (e.g. a project shipped or metric achieved).`,
            priorityLevel: 'medium',
    ...
    if (input.extractedSkills.length < 8) {
        results.push({
            ...
            description: `Only ${input.extractedSkills.length} skill(s) detected by the ATS engine. List specific technologies and tools — e.g. "Vertex AI, LangChain, AWS Bedrock, FastAPI, PostgreSQL" — to improve keyword density and pass automated screening filters.`,
            priorityLevel: 'medium',
    ...
    if (!/linkedin\.com/i.test(input.resumeText)) {
        results.push({
            ...
            description: 'No LinkedIn URL found in your resume. Recruiters almost always verify candidates on LinkedIn before scheduling interviews — add your full profile URL (e.g. linkedin.com/in/yourname) to the contact section.',
            priorityLevel: 'low',
    ...
    if (!/certif(ication|ied)/i.test(input.resumeText)) {
        results.push({
            ...
            description: 'No certifications section detected. For AI, cloud, and data roles, certs like AWS Solutions Architect, Google Cloud Professional, or TensorFlow Developer are strong hiring signals that validate your expertise objectively.',
            priorityLevel: 'low',
    ...
    if (input.totalWords < 100) {
        results.push({
            ...
            description: `Your resume contains only ${input.totalWords} words — recruiters expect 400–600 words on a single page. Thin resumes signal a lack of depth. Expand your experience bullets, add a projects section, and include a skills list to reach the target length.`,
            priorityLevel: 'high',
    ...
    if (specificityCheck.averageScore < 3 && specificityCheck.bullets.length > 0) {
        ...
        results.push({
            ...
            description: `${weakCount} bullet point(s) score 1–2 out of 5 on specificity. Vague bullets like "Worked on AI projects" are skipped by recruiters. Rewrite them with the formula: [Action verb] + [what you built/did] + [technology used] + [measurable outcome].`,
